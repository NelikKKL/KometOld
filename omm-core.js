...
class OmmModel extends HTMLElement {
    constructor() {
        ...
        this._autoRotate = false; // set by 'autorate' keyword in content
    ...
    parse(txt) {
        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            // autorate keyword on its own line enables auto-rotation
            if (l === 'autorate') {
                this.isAnimating = true;
    ...
    getImg(src) {
        // Block base64 data URIs — only external URLs allowed
        if (src.startsWith('data:')) {
            console.warn('OMM: base64 textures are not supported. Use a URL instead.');
            return null;
        }
    ...
    animate() {
        ...
        if (this.hasAttribute('autorate') || this._autoRotate) this.camera.ry += 0.01;
